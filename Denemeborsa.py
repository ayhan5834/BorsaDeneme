...
def grafik_verisi_indir(sorgu, periyot="1mo", aralik="1d"):
    """Yahoo Finance üzerinden belirtilen hisse kodunun geçmiş verilerini indirir.

    sorgu: Hisse kodu (örn: 'THYAO.IS') periyot: Grafik veri aralığı (1mo: 1 ay,
    5d: 5 gün, 1y: 1 yıl) aralik: Mumların zaman dilimi (1d: 1 günlük, 1h: 1
    saatlik)
    """
    try:
        # yfinance ile veriyi çekiyoruz (Son 1 aylık günlük veriler idealdir)
        hisse = yf.Ticker(sorgu)
        df = hisse.history(period=periyot, interval=aralik)

        # Eğer veri boş değilse döndür
        if not df.empty:
            return df
        else:
            return pd.DataFrame()  # Boş veri seti döndür
    except Exception as e:
