...
if IS_STREAMLIT:   
    ...
    # --- 3. SEKME: MEGA RADAR TARAMASI (DÜZELTİLMİŞ) ---
    with sekme3:
        st.subheader("🔍 Mega Radar Taraması")
        st.write("Tüm BIST hisseleri taranarak AL sinyali üretenler listelenir.")
        
        # HİSSE LİSTESİNİ BURADA YENİDEN ÇAĞIRIN (Streamlit için güvenli hale getirin)
        ...
        if st.button("🚀 TÜM BORSAYI TARAMAYA BAŞLAT", key="mob_radar_start"):
            ...
            toplam = len(guncel_hisse_listesi) # TUM_BIST yerine yerel değişkeni kullanın
            ...
            for idx, h in enumerate(guncel_hisse_listesi):
                ...
                try:
                    # Buradaki formatın '.IS' eklenmiş haliyle doğru olduğundan emin olun
                    df = yf.download(h + ".IS", period="40d", interval="1d", progress=False)
                    
                    if df is None or df.empty or len(df) < 15: 
                        continue
                    
                    # ... [Analiz mantığı buraya devam eder] ...
